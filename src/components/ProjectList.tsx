      ...
      <div className="space-y-1">
        {filtered.map((p) => (
          <div
            key={p.id}
            className={cn(
              'group flex items-center justify-between py-3 cursor-pointer transition-all duration-300',
              selectedProjectId === p.id ? 'opacity-100 pl-4 border-l-2 border-black dark:border-white' : 'opacity-60 hover:opacity-100 hover:pl-2'
            )}
            onClick={() => onSelect(p)}
          >
            <div className="min-w-0">
              <div className="flex items-center gap-3">
                <div className={cn('w-1.5 h-1.5 rounded-full transition-transform group-hover:scale-125',
                  p.status === 'active' ? 'bg-black dark:bg-white' : 'bg-neutral-400'
                )} />
                <div className={cn("text-sm uppercase tracking-wide truncate transition-all", selectedProjectId === p.id ? "font-medium" : "font-normal")}>
                  {p.name}
                </div>
              </div>
            </div>
